...
function convertNode(htmlRoot) {
  return plugins.walkTree(htmlRoot, (htmlNode, next2) => {
    const node = {
...
async function loadPlugins(plugins$1 = ["katex", "hljs"]) {
  const allStyles = [];
  const allScripts = [];
  ...
  await plugins.loadCSS(allStyles);
  await plugins.loadJS(allScripts);
  const loaded = {};
